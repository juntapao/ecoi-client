...
@section('content')
    <div class="card">
                    ...
                    @if(count($users) > 0)
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td>{{ isset($user->branch) ? $user->branch->branch_name : null }}</td>
                                <td class="text-center">
                                    <a href="{{ route('users.edit', $user->id) }}" class="btn btn-sm btn-warning loading" >Edit</a>  
                                    <a href="{{ route('users.show', $user->id) }}" class="btn btn-sm btn-success loading" >Show</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
        ...
        <div class="card-footer py-4">
            <div class="row">
                <div class="col text-left">Showing {{ $users->count() }} out of {{ number_format($users->total()) }} record(s)</div>
                <div>{{ $users->links() }}</div>
            </div>
        </div>
    </div>
