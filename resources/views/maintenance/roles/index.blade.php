...
@section('content')
    <div class="card">
                        ...
                        @foreach($roles as $role)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a href="{{ route('roles.edit', $role->id) }}" class="btn btn-sm btn-warning loading" >Edit</a>  
                                    <a href="{{ route('roles.show', $role->id) }}" class="btn btn-sm btn-success loading" >Show</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
        ...
        <div class="card-footer py-4">
            <div class="row">
                <div class="col text-left">Showing {{ $roles->count() }} out of {{ $roles->total() }} record(s)</div>
                <div>{{ $roles->links() }}</div>
            </div>
        </div>
    </div>
