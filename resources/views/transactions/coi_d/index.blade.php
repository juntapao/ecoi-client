    ...
    <div class="row align-items-center py-4">
        <div class="col-7 col-lg-6">
            ...
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                ...
                <li class="breadcrumb-item active" aria-current="page">Family Protect</li>
                </ol>
            </nav>
        </div>
                            ...
                            <tr @if($transaction->status == 'edited') class="table-primary" data-toggle="tooltip" data-placement="bottom" title="This record has been modified" @endif>
                                ...
                                <td class="text-center">
                                    @if(!$transaction->posted)
                                        <a href="{{ route('coi_d.edit', Crypt::encrypt($transaction->id)) }}" class="btn btn-sm btn-warning loading" >Edit</a>  
                                        <a href="{{ route('coi_d.post', Crypt::encrypt($transaction->id)) }}" onclick="return confirm('Are you sure you want to Post this transaction?')" class="btn btn-sm btn-primary" >Post</a>
                                    @else
                                        <a href="{{ route('coi_d.edit', Crypt::encrypt($transaction->id)) }}" class="btn btn-sm btn-warning disabled" disabled>Edit</a>  
                                        <a href="{{ route('coi_d.print', Crypt::encrypt($transaction->id)) }}" target="_blank" class="btn btn-sm btn-danger" >Print</a>
                                    @endif
                                    <a href="{{ route('coi_d.show', Crypt::encrypt($transaction->id)) }}" class="btn btn-sm btn-success loading" >Show</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
