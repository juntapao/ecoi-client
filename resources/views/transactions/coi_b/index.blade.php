                            ...
                            <tr @if($transaction->status == 'edited') class="table-primary" data-toggle="tooltip" data-placement="bottom" title="This record has been modified" @endif>
                                ...
                                <td class="text-center">
                                    @if($transaction->posted)
                                        <a href="{{ route('coi_b.edit', $transaction->id) }}" class="btn btn-sm btn-warning disabled" disabled>Edit</a>  
                                        <a href="{{ route('coi_b.print', $transaction->id) }}" target="_blank" class="btn btn-sm btn-danger" >Print</a>
                                    @else
                                        <a href="{{ route('coi_b.edit', $transaction->id) }}" class="btn btn-sm btn-warning loading" >Edit</a>  
                                        {{-- <a href="{{ route('coi_b.post', $transaction->id) }}" onclick="return confirm('Are you sure you want to Post this transaction?')" class="btn btn-sm btn-primary" >Post</a> --}}
                                        <a href="{{ route('coi_b.show', $transaction->id) }}" class="btn btn-sm btn-primary loading">Post</a>
                                    @endif
                                    <a href="{{ route('coi_b.show', $transaction->id) }}" class="btn btn-sm btn-success loading" >Show</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
