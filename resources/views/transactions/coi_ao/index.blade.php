    ...
    <div class="row align-items-center py-4">
        <div class="col-7 col-lg-6">
            ...
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                ...
                <li class="breadcrumb-item active" aria-current="page">Kwarta Padala Protect</li>
                </ol>
            </nav>
        </div>
                    ...
                    @if(count($transactions) > 0)
                        @foreach($transactions as $transaction)
                            <tr @if($transaction->status == 'edited') class="table-primary" data-toggle="tooltip" data-placement="bottom" title="This record has been modified" @endif>
                                ...
                                <td>{{$transaction->kpt_number}}</td>
                                <td>{{$transaction->policy_number}}</td>
                                <td>{{$transaction->insured_name}}</td>
                                <td class="text-center">
                                    @if(!$transaction->posted)
                                        <a href="{{ route('coi_ao.edit', Crypt::encrypt($transaction->id)) }}" class="btn btn-sm btn-warning loading" >Edit</a>  
                                        <a href="{{ route('coi_ao.post', Crypt::encrypt($transaction->id)) }}" onclick="return confirm('Are you sure you want to Post this transaction?')" class="btn btn-sm btn-primary" >Post</a>
                                    @else
                                        <a href="{{ route('coi_ao.edit', Crypt::encrypt($transaction->id)) }}" class="btn btn-sm btn-warning disabled" disabled>Edit</a>  
                                        <a href="{{ route('coi_ao.print', Crypt::encrypt($transaction->id)) }}" target="_blank" class="btn btn-sm btn-danger" >Print</a>
                                    @endif
                                    <a href="{{ route('coi_ao.show', Crypt::encrypt($transaction->id)) }}" class="btn btn-sm btn-success loading" >Show</a>
                                </td>
                            </tr>
                        @endforeach
                    @else
